...
/// </summary>
public static class UsePageMiddlewareExtensions
{
    /// <summary>
    ...
    public static IPageBuilder UseMiddleware<TMiddleware>(this IPageBuilder app, params object[] args) =>
    ...
    /// <param name="args">The arguments to pass to the middleware type instance's constructor.</param>
    /// <returns>The <see cref="IApplicationBuilder"/> instance.</returns>
    public static IPageBuilder UseMiddleware(this IPageBuilder app, Type middleware, params object[] args)
    {
        if (typeof(IPageMiddleware).GetTypeInfo().IsAssignableFrom(middleware.GetTypeInfo()))
        {
    ...
    private static IPageBuilder UseMiddlewareInterface(IPageBuilder app, IServiceProvider services, Type middlewareType) =>
        app.Use(next =>
            context =>
            {
                var middleware = (IPageMiddleware)services.GetRequiredService(middlewareType);
                if (middleware == null)
                {
